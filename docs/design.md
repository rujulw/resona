...
## Discord Activity Vinyl Color Scheme

### Slot scheme

24 hue slots at 15° increments, covering the full hue wheel.
...
`vinyl_default` is used when a track has no artwork or the artwork has no sufficiently saturated pixels.

### Asset spec

- Format: PNG, 512×512 px, uploaded once to Discord Developer Portal
- Disc fill: `#050505`
- Groove rings: `repeating-radial-gradient` `#1a1a1a / #000` at 40% opacity
- Specular overlay: conic gradient white highlights at 30% opacity
- Center label: 30% of diameter (~154 px), color `hsl({hue}, 62%, 38%)`
- Spindle: 8 px circle, `rgba(255,255,255,0.12)` at center
- No tonearm (it extends outside the square boundary in the UI)

Generated by `scripts/generate-discord-assets.mjs`. Upload manually; keys must match the table.

### Extraction algorithm (Rust, `server/src/presence/color.rs`)

1. Resolve artwork path: `<app_data_dir>/artwork/<artwork_key>`
2. Decode image bytes → RGBA via `image` crate
3. Sample every 8th pixel
4. Convert RGB → HSL; reject if saturation < 0.25 or lightness outside [0.15, 0.85]
5. Bucket qualifying pixels into 24 hue slots (`floor(hue° / 15)`)
6. Pick slot with max count; return `None` if fewer than 20 qualifying pixels
7. Result cached in `PlaybackState` — not re-extracted per presence sync

### Data flow

```
load_playback_track command
  → query DB for track artwork_key
  → store in PlaybackState (internal field)

sync_presence_with_snapshot
  → if vinyl_color_slot is None and artwork_key is Some
      → dominant_hue_slot(artwork_path) → cache slot
  → PresencePayload.large_image = slot_to_asset_key(slot)
  → Discord activity large_image asset key
```
